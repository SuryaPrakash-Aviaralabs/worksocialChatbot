...
export const DateExtension = {
  ...
  render: ({ trace, element }) => {
    ...
    form.innerHTML = `
      ...
      <input type="tel" id="phone" required pattern="\\d{7,}" title="Enter at least 10 digits"/><br>

      <label>Select a date</label>
      <input type="date" id="appointmentDate" min="${minDateString}" max="${maxDateString}" /><br>

      <label>Select a time Slot</label>
      <input type="time" id="appointmentTime" list="time-slots" step="1800" /><br>

      <small class="error-msg" id="error-msg">Please correct the highlighted fields.</small><br>
    ...
    function validate() {
      ...
      if (!dateVal || !timeVal) datetimeValid = false;
        ...
        if (h < 9 || (h === 9 && mi < 30) || h > 17) datetimeValid = false;
      }
      ...
      timeInput.classList.toggle("invalid", !datetimeValid);
    ...
    form.addEventListener("submit", (e) => {
      ...
      const display = fullDate.toLocaleString("en-US", {
        weekday: "short",
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
      ...
      // Disable fields
      [name, email, phone, dateInput, timeInput].forEach(el => el.disabled = true);
      submitButton.remove();

      const success = document.createElement("div");
      success.className = "success-msg";
      success.textContent = `✅ Tour scheduled for ${display}`;
      ...
      window.voiceflow.chat.interact({
        ...
        payload: {
          ...
          email: email.value,
          phone: phone.value,
          appointmentDateTime: `${dateVal}T${timeVal}`,
          date: dateVal,
          time: timeVal,
          weekday,
          display,
